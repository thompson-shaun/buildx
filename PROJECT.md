...
- **Familiar user experience:** Buildx offers a user exerpeince simliar to legacy docker build, ensuring a smooth transition from legacy commands
- **Full BuildKit capablities:** Leverage the full feature set of [`moby/buildkit`](https://github.com/moby/buildkit) when using the container driver
...
- **Compose integration:** Build complex, mulit-services files as defined in compose
- **High-level build constructs via `bake`:** Introduces high-level build constructs for more complex build workflows
- **In-container driver support:** Support in-container drivers for both Docker and Kubernetes environemts to support isolation/security.
...
| `dependencies`      | All         | Addressing project dependencies                                           |
...
Users can expect 2-3 release candidate (RC) test releases prior to a feature release. The first RC is typically released about two weeks before the final release.

#### API Compatibility

Buildx ensures backward compatibility in the gRPC API with previous versions, allowing older clients to interact with newer daemons and vice versa. Features marked for removal are first deprecated in a feature release. We commit to maintaining backward compatibility for certain APIs, such as the LLB API, and avoid making backward-incompatible changes. Internally, Buildx APIs use feature detection to ascertain supported features on both sides of the API.

#### Go Modules

Go modules in the Buildx repository are not guaranteed to be backward compatible with previous release branches.
...
- The `v0.next` milestone is used to gather all items that are being planned for some arbitrary upcoming release
- The `v0.future` milestone is used to gather all triaged items that are not fit for a future release due to some condition. These items can be blocked for some reason but will need to be unblocked before leaving the backlog.
...
- Describe the issue's intent and purpose precisely. This is necessary because it can be difficult for an issue to explain how an end user experiences a problem and what actions they took to arrive at the problem.
...
This is the default priority and is implied in the absence of a `prioirty/` label. Bugs with this priority should be included in the next feature release but may land in a patch release if they are ready and unlikely to impact other functionality adversely. Non-bug issues with this priority should also be included in the next feature release if they are available and ready.
...
To help expedite common operations, avoid errors and reduce toil some workflow automations are used by maintainers. These include:
